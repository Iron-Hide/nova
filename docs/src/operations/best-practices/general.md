---
title: Nova Validator Operations Best Practices
...
From within the Solana CLI, you can execute the `nova-validator` command with
...
Assuming you are ready to restart, you may use the `nova-validator exit`
...
It is a best practice to always build your Nova binaries from source. If you
...
your `nova-validator` binary to your specific hardware.
...
`nova-install` command.

Assuming you want to install Nova version `2.0.15`, you would execute the
following:

```
nova-install init 2.0.15
```

This command downloads the executable for `2.0.15` and installs it into a
`.local` directory. You can also look at `nova-install --help` for more
options.
...
the newly installed version is in use. Use `nova-validator exit` to restart
...
`nova-validator` command:

```
--no-snapshot-fetch
```

If you use this flag with the `nova-validator` command, make sure that you run
...
running the `nova-validator` software. How much SOL you should store there is
...
> **Note** `nova-watchtower` can monitor for a minimum validator identity
