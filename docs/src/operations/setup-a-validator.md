---
title: Setup an Nova Validator
sidebar_label: Setup an Nova Validator
sidebar_position: 5
---
...
Your remote machine will need the Solana CLI installed to run the Nova validator
...
exec nova-validator \
    ...
    --only-known-rpc \
    --log /home/sol/nova-validator.log \
    --ledger /mnt/ledger \
...
Refer to `nova-validator --help` for more information on what each flag is
...
The script should execute the `nova-validator` process. In a new terminal
window, ssh into your server, then verify that the process is running:

```
ps aux | grep nova-validator
```

You should see a line in the output that includes `nova-validator` with all
...
```
tail -f /home/sol/nova-validator*.log
```

## Monitoring

`nova-watchtower` is a command you can run on a separate machine to monitor
your server. You can read more about handling
[automatic restarts and monitoring](./best-practices/monitoring.md#nova-watchtower)
