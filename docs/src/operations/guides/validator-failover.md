...
### Primary Validator
The following additional `nova-validator` parameters are required to enable
...
following `nova-validator` command-line argument changes:
...
```bash
$ nova-validator wait-for-restart-window --identity validator-keypair.json \
  && nova-validator set-identity validator-keypair.json
```
...
Monitoring of your choosing can invoke the `nova-validator set-identity
