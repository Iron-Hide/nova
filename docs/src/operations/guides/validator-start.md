...
the `--cuda` argument to `nova-validator`.
...
```bash
sudo bash -c "cat >/etc/sysctl.d/21-nova-validator.conf <<EOF
...
```bash
sudo sysctl -p /etc/sysctl.d/21-nova-validator.conf
```
...
`nova-validator`. You can specify multiple ones by repeating the argument
...
> `nova-validator --identity ASK ... --authorized-voter ASK ...`
...
example, `nova-validator --dynamic-port-range 11000-11020 ...` will restrict
...
`nova-validator` command-line. Ensure that the 'exec' command is used to
start the validator process (i.e. "exec nova-validator ..."). This is
...
The validator log file, as specified by `--log ~/nova-validator.log`, can get
...
launch the process with `exec` (`exec nova-validator ...`) when using logrotate.
...
nova-validator ..."); otherwise, when logrotate sends its signal to the
...
Now add the `--accounts /mnt/solana-accounts` argument to your `nova-validator`
