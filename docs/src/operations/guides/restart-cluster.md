...
In Nova 1.14 or greater, run the following command to output the latest
optimistically confirmed slot your validator observed:
```bash
nova-ledger-tool -l ledger latest-optimistic-slots
```

In Nova 1.13 or less, the latest optimistically confirmed can be found by looking for the more recent occurrence of
...
```bash
$ nova-ledger-tool -l <LEDGER_PATH> --snapshot-archive-path <SNAPSHOTS_PATH> --incremental-snapshot-archive-path <INCREMENTAL_SNAPSHOTS_PATH> create-snapshot SLOT_X <SNAPSHOTS_PATH> --hard-fork SLOT_X
```

The snapshots directory should now contain the new snapshot.
`nova-ledger-tool create-snapshot` will also output the new shred version, and bank hash value,
...
> ```
>
>      You can check for which slots your ledger has with: `nova-ledger-tool -l path/to/ledger bounds`
>
...
```bash
$ nova-ledger-tool -l ledger create-snapshot SLOT_X ledger --hard-fork SLOT_X \
