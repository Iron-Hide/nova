...
Operate a configured testnet
 ...
 sanity/start-specific options:
   ...
   -o noInstallCheck    - Skip nova-install sanity
 ...
 update-specific options:
   --platform linux|osx|windows       - Deploy the tarball using 'nova-install deploy ...' for the
...
deployUpdate() {
  ...
  for updatePlatform in $updatePlatforms; do
    echo "--- Deploying nova-install update: $updatePlatform"
    (
      set -x

      scripts/nova-install-update-manifest-keypair.sh "$updatePlatform"
