...
if [[ ! -f update_manifest_keypair.json ]]; then
  "$SOLANA_ROOT"/scripts/nova-install-update-manifest-keypair.sh "$OS"
fi
...
edge|beta)
  ...
  ;;
*)
  DOWNLOAD_URL=https://github.com/anza-xyz/nova/releases/download/"$TAG"/solana-release-$TARGET.tar.bz2
  ;;
esac
...
# shellcheck disable=SC2086 # Don't want to double quote $maybeKeypair
nova-install deploy $maybeKeypair --url "$URL" "$DOWNLOAD_URL" update_manifest_keypair.json
