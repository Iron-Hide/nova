...
(
  ...
  cp "${RELEASE_BASENAME}"/bin/nova-install-init nova-install-init-$TARGET
...
for file in "${TARBALL_BASENAME}"-$TARGET.tar.bz2 "${TARBALL_BASENAME}"-$TARGET.yml nova-install-init-"$TARGET"* $MAYBE_TARBALLS; do
