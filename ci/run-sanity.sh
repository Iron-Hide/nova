...
if [[ -z $CI ]]; then
  ...
  $solana_cli --version
  $nova_validator --version
  $solana_ledger_tool --version
fi

rm -rf config/run/init-completed config/ledger

# Sanity-check that nova-validator can successfully terminate itself without relying on
...
$nova_validator --ledger config/ledger exit --force || true
