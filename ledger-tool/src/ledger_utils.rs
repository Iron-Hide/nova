...
pub fn load_and_process_ledger(
    ...
    let account_paths = if let Some(account_paths) = arg_matches.value_of("account_paths") {
        // If this blockstore access is Primary, no other process (nova-validator) can hold
        ...
        if !blockstore.is_primary_access() {
            ...
            // the Primary access instead of holding it to give priority to nova-validator over
            // nova-ledger-tool should nova-validator start before we've finished.
            info!(
